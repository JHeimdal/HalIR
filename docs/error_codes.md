...
| Function | `0` | `1` | `2` |
|---|---|---|---|
| `halir_simulation_setup_set_project` | Success | Invalid argument (NULL) | Bounded string copy failed |
| `halir_simulation_setup_add_project_file` | Success | Invalid argument | Allocation failed |
| `halir_simulation_setup_set_sample_env` | Success | Validation failure | Unit conversion or string assignment failure |
| `halir_simulation_setup_add_composition` | Success | Invalid argument | Allocation or bounded copy failure |
...
| `halir_simulation_setup_validate` | Valid workspace | Validation failure | N/A |
...
- Keep cleanup deterministic: call `halir_simulation_setup_free` on all failure paths after workspace creation.
